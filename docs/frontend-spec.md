...
- 提问调用 `POST /kb/qa`。
- 回答必须展示来源。
- 连续追问时携带最近对话历史，便于后端理解“那截止时间呢”等省略主语的问题。
- 来源展示为可点击入口，优先跳转知识条目；仅有文件地址时触发文件下载。
- 回答展示置信度和可靠性提示，低置信度或无来源时必须提示用户该回答无知识库引用。
- 没有可靠来源时，若后端返回通用回答则按低置信度展示；若后端返回“未检索到可靠依据”则原样展示。
- 模板下载调用文件下载接口。
