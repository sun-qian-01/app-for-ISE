...
- 前后端分离原则：前端负责页面展示和交互，后端负责鉴权、业务规则、状态流转和数据持久化。
- 后端强校验原则：前端隐藏按钮不等于安全控制，所有权限、数据范围和对象归属必须由后端校验。
- 单一数据来源原则：状态、枚举、权限码、接口路径以 `docs/dictionary.md`、`docs/api.md` 和后端实现为准。
- 可追溯原则：关键业务操作写审计日志，系统异常写系统事件日志。
- 敏感字段最小化原则：身份证号、联系方式、户籍地、导师、修学/延毕记录等字段默认脱敏，完整查看需权限并记录审计。
- 模块化原则：按用户权限、学生画像、知识库、党团流程、通知、申请审批、荣誉、日志等业务域拆分。
- 渐进扩展原则：P0/P1 功能作为一期主线，P2 学业预警和理论自测保留扩展位。
...
| 层次 | 约束 |
| --- | --- |
| 前端 | 正式项目为 Web 网站，建议 Vue 3 + Vite 或 React + Vite。 |
| 后端 | 提供 RESTful JSON API，统一路径前缀 `/api/v1`。 |
| 认证 | 登录后返回 token，后续通过 `Authorization: Bearer <token>` 传递。 |
| 数据库 | SQL 按 Kingbase/PostgreSQL 兼容语法设计。 |
| 文件上传 | 上传接口统一返回 `fileId`，业务接口只传文件 ID。 |
...
- 角色、权限、数据范围应可配置。
- 知识库分类、通知标签、申请类型、荣誉类别等枚举应统一维护。
- 党团流程阶段应通过流程定义和阶段定义维护。
- 证明、请假、盖章等院内申请应通过模板和表单配置扩展。
- 学业预警作为 P2 扩展，不影响一期主流程。
